  ...
  const getStatusBadge = (status: QueueJob['status']) => {
    switch (status) {
      ...
      case 'awaiting_manual_upload':
        return (
          <span className="bg-[#4DFFE0]/10 text-[#4DFFE0] border border-[#4DFFE0]/30 px-2 py-0.5 rounded-full text-[11px] font-mono inline-flex items-center gap-1" title="Instagram Challenge! Copiado a /sdcard/Download vía ADB">
            <AlertTriangle className="w-3 h-3 text-[#4DFFE0]" /> Fallback ADB Push
          </span>
        );
      case 'failed':
        return (
    ...
    <div className="bg-[#101A2D] border border-[#1E2C42] rounded-xl flex flex-col h-full overflow-hidden shadow-xl">
      ...
      <div className="p-3 bg-[#0B1320] border-b border-[#1E2C42]">
        <form onSubmit={handleSubmit} className="flex flex-wrap items-center gap-2 font-mono">
          ...
          <button
            type="submit"
            className="bg-[#1E293B] hover:bg-[#334155] text-[#00E5BE] border border-[#4DFFE0]/30 px-3 py-2 rounded-lg text-xs font-mono flex items-center gap-1 transition-colors"
          >
                ...
                <tr key={job.id} className="hover:bg-[#0B1320] transition-colors">
                  ...
                  <td className="py-2.5 px-3 text-white cursor-pointer" onClick={() => onOpenPreview && onOpenPreview(job)}>
                    <div className="flex items-center gap-1.5 font-medium hover:text-[#00E5BE] transition-colors">
                      <Sparkles className="w-3.5 h-3.5 text-[#4DFFE0]" />
                      {job.keyword}
                    </div>
                  </td>
                  ...
                  <td className="py-2.5 px-3 text-right">
                    <button
                      ...
                      className="px-2.5 py-1 bg-[#1E293B] hover:bg-[#00E5BE]/20 hover:border-[#00E5BE] border border-[#4DFFE0]/40 rounded-lg text-[11px] text-[#4DFFE0] hover:text-[#00E5BE] font-bold transition-all inline-flex items-center gap-1 shadow-sm"
